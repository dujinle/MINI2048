...
let WxGlobal = {
    ...
    registOnShow: function () {
        wx.onShow((res) => {
            ...
            if (this.sharePointConfig.wait) {
                ...
                if (Date.parse(d) - Date.parse(this.sharePointConfig.shareTime) > Global.cdnGameConfig.minShareTime * 1000) {
                    // 大于最小分享时间
                    console.log("大于最小分享时间");

                    let successWeightIndex = Global.shareTimes % Global.cdnGameConfig.shareSuccessWeight.length;
                    let successWeight = Global.cdnGameConfig.shareSuccessWeight[successWeightIndex];
                    let isSuccess = Math.random() < successWeight;

                    console.log("分享成功概率：", isSuccess, Global.shareTimes, successWeightIndex, successWeight);
    ...
    getShareInfo: function () {
        var imageUrl;
        var text;
        var shareIndex = util.getRandomIndexForArray(Global.cdnShareImages);
        if (shareIndex > -1) {
            imageUrl = Global.cdnShareImages[shareIndex];
            text = Global.cdnTexts[shareIndex];
        } else {
            shareIndex = util.getRandomIndexForArray(Global.shareImages);
            if (shareIndex > -1) {
                var imageName = Global.shareImages[shareIndex];
                imageUrl = 'res/raw-assets/resources/textures/' + imageName;
                text = Global.shareTexts[shareIndex];
            }
            console.log('获取cdn分享图错误，读取本地分享数据');
        }
        return {
            imageUrl: imageUrl,
            text: text
    ...
    saveCloudData: function () {
        ...
        if (!Global.gameCloudInfo) return;
        //存储key-value格式数据到微信云端
        var maxscore = parseInt(Global.gameCloudInfo.maxscore);
        var gold = parseInt(Global.gameCloudInfo.gold);
